...
import { Calendar as CalendarIcon, PlayCircle, CheckCircle2 } from "lucide-react";
...
export default function CalendarioPage() {
  ...
  const tarefasFiltradas = useMemo(() => {
    ...
    if (selectedProducts.length > 0) {
      filtradas = filtradas.filter((d) => d.produto && selectedProducts.includes(d.produto));
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Visão geral dos prazos e produtividade
            </p>
          </div>
        </div>
...
function StatusCard({ label, value, icon, color }) {
  ...
  return (
    <div
      className={`p-6 rounded-2xl border-2 ${c.bg} ${c.border} flex items-center justify-between transition-transform hover:scale-[1.02]`}
    >
